...
function dsk.read(filename)
    ...
    header = dsk.datafile:read(34) or ""
    if (string.sub(header, 1, 8) == "MV - CPC") then
        dsk.version = 1
    else
        if (string.sub(header, 1, 16) == "EXTENDED CPC DSK") then
            dsk.version = 5
        else
    ...
    if ((counttracks ~= dsk.tracksnumber) and (dsk.version ~= 1)) then
        if (dsk.verbose == true) then
            print("DSK Fileformat error : " ..
                dsk.tracksnumber .. " tracks announced altough " .. counttracks .. " are listed in the directory")
        end
    ...
    if (dsk.verbose == true) then
        print("Opening file : " ..
            filename ..
            " [Creator:'" ..
            creator .. "'/Tracks:" .. dsk.tracksnumber .. "/Sides:" .. dsk.sidesnumber .. "/Track size:" ..
            dsk.tracksize .. "]")
    end
    ...
    do
        ...
        if (dsk.verbose == true) then
            print("Track : " ..
                ...
                sidenum ..
                " / sectors size : " ..
                dsk.tracks[tracknum][sidenum].sectorssize ..
                " / nb of sectors : " .. dsk.tracks[tracknum][sidenum].sectorsnumber)
        end
        ...
        do
            ...
            if (dsk.verbose == true) then
                print("Sector num : " ..
                    cpt_sectors ..
                    " / id : " ..
                    string.format("#%02x", dsk.tracks[tracknum][sidenum].sector[cpt_sectors].id) ..
                    " / size : " ..
                    dsk.tracks[tracknum][sidenum].sector[cpt_sectors].size ..
                    " / actual size : " .. dsk.tracks[tracknum][sidenum].sector[cpt_sectors].actualsize)
            end
        end
    ...
    do
        ...
        do
            ...
            do
                ...
                dsk.tracks[cpt_tracks][cpt_sides].sector[cpt_sectors].data = string.rep(
                    string.char(dsk.tracks[cpt_tracks][cpt_sides].filler),
                    256 << (dsk.tracks[cpt_tracks][cpt_sides].sector[cpt_sectors].size - 1))
            end
        end
    end
...
function dsk.initializefreeblocks()
    if (dsk.verbose == true) then
        ...
        print("Nb tracks = " ..
            dsk.tracksnumber .. ", tracksize = " ..
            dsk.tracksize .. " ==> blocks = " .. (((dsk.tracksnumber * dsk.tracksize) >> 10) - 2))
    end
    dsk.freeblocks = {}
...
function dsk.cat()
    ...
    for entrynum = 0, 63, 1 do
        if (string.byte(string.sub(directory, entrynum * 32 + 1, entrynum * 32 + 1)) ~= 0x0E5) then
            ...
            for blocks = 1, nbblockstoread, 1 do
                dsk.catalog[cptentry].blocks[blocks] = string.byte(directory, entrynum * 32 + 16 + blocks,
                    entrynum * 32 + 16 + blocks)
    ...
    if (dsk.verbose == true) then
        for num, direntry in pairs(dsk.catalog) do
            io.write(num ..
                " : " ..
                direntry.user .. " " .. direntry.numextension .. " " .. direntry.filename ..
                " " .. #direntry.blocks .. " blocks (")
...
function dsk.writecatalog() -- Writes the catalog on the tracks of the dsk
    ...
    for num, catalogentry in pairs(dsk.catalog) do
        if (type(catalogentry) == 'table') then
            if (dsk.verbose == true) then
                io.write(catalogentry.user ..
                    " " .. catalogentry.numextension .. " " .. catalogentry.filename ..
                    " " .. #catalogentry.blocks .. " blocks (")
...
function dsk.deletefile(filename)
    ...
    table.sort(dsk.catalog,
        function(k1, k2)
            if (k1.key == k2.key) then
                return k1.numrecord < k2.numrecord
            else
                return k1.filename <
                    k2.filename
            end
...
function dsk.generateheader(user, filename, filetype, loadaddr, entryaddr, length)
    local header = string.char(user)
        ...
        .. string.char(0, 0, 0, 0, 0, 0, filetype, 0, 0, loadaddr & 255, loadaddr >> 8, 0, length & 255, length >> 8,
            entryaddr & 255, entryaddr >> 8)
...
function dsk.populateheader(header, user, filename, filetype, loadaddr, entryaddr, length)
    ...
    headerpatched2 = string.sub(headerpatched, 1, 0x43) ..
        string.char(checksum & 255) .. string.char(checksum >> 8) .. string.sub(headerpatched, 0x46)
...
function dsk.save(filename, filetype, frombyte, tobyte, entryaddr)
    ...
    amsdosfilename = string.sub(amsdosfilename, 1, pointpos - 1) ..
        string.rep(" ", 9 - pointpos) .. string.sub(amsdosfilename, pointpos - string.len(amsdosfilename))
...
function dsk.savewithcustomheader(header, filename, filetype, frombyte, tobyte, entryaddr)
    ...
    local blockdata = dsk.populateheader(header, 0, amsdosfilename, filetype, frombyte, entryaddr, string.len(data)) ..
        data
