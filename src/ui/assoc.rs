...
pub(super) fn render_lua_app_palette(f: &mut Frame, s: &LuaAppPaletteState, area: Rect) {
    let matches = s.filtered_indices();
    let w: u16 = area.width.saturating_sub(4).min(112).max(64);
    let visible = (matches.len() as u16).min(12).max(5);
    let h: u16 = (visible + 7).min(area.height.saturating_sub(4)).max(10);
    let popup = clamp_rect(
        area,
        Rect {
            x: area.x + (area.width.saturating_sub(w)) / 2,
    ...
    let count_hint = if s.query.is_empty() {
        format!(" {}/{} ", s.match_pos.saturating_add(1), s.items.len())
    } else if matches.is_empty() {
        " 0/0 ".to_string()
    } else {
        format!(" {}/{} ", s.match_pos.saturating_add(1), matches.len())
    };
    let hint_w = count_hint.len() as u16;
    let input_w = inner.width.saturating_sub(hint_w) as usize;
    let input_text = format!(" ⌕ {}▁", s.query);
    ...
    let list_h = list_area.height as usize;
    let start = if s.match_pos >= list_h && list_h > 0 {
        s.match_pos - list_h + 1
    } else {
        0
    };

    for (idx, match_idx) in (start..matches.len()).take(list_h).enumerate() {
        let item = &s.items[matches[match_idx]];
        let selected = match_idx == s.match_pos;
        let style = if selected {
            Style::default()
                .fg(CLR_QS_SEL_FG)
                .bg(CLR_QS_SEL_BG)
        ...
        let text = format!(
            ...
            item.version,
            truncate_str(&item.description, (inner.width as usize).saturating_sub(36))
        );
        let padded = format!("{:<width$}", text, width = inner.width as usize);
        safe_render_widget(
            f,
