...
pub fn handle_event(app: &mut App, event: Event) -> Result<bool> {
    ...
    match &app.mode {
        ...
        AppMode::Terminal => return handle_terminal(app, key),
        AppMode::Browse => {}
    }
...
// ---------------------------------------------------------------------------
// Ctrl-U / Esc pseudo-terminal mode
// ---------------------------------------------------------------------------

fn handle_terminal(app: &mut App, key: KeyEvent) -> Result<bool> {
    let ctrl = key.modifiers.contains(KeyModifiers::CONTROL);
...
fn terminal_execute(app: &mut App) -> Result<bool> {
    ...
    match cmd {
        ...
        "cd" => {
            let panel_dir = match app.active {
                crate::app::ActivePanel::Left  => app.left.path.clone(),
                crate::app::ActivePanel::Right => app.right.path.clone(),
            };
    ...
    app.running_cmd = Some(crate::app::spawn_cmd_streaming(raw, panel_dir));
    Ok(false)
}
